...
def get_container(config: Dict) -> containers.DynamicContainer:
    ...
    logger = providers.Singleton(LocalLogger, config=config)
    container.logger = logger
    bot_manager = providers.Singleton(LocalBotManager, config=config, logger=logger)
    exchange_manager = providers.Singleton(
        CctxExchangeManager, config=config, logger=logger
    )
